...
def bad_pixels(data):
    ...
    data = np.array(data, float) #TODO________________________Double Check This?
...
def all_within_diameter_are_good(data, diameter):
    ...
    for c in range(N):    
        j,i = coords[c]
        x = (x_arr + i).astype(int).clip(0, datax-1)
        y = (y_arr + j).astype(int).clip(0, datay-1)
        mask[y, x] = 0 
...
def getMask(data, smr=SMR, wlen=WLEN):
    ...
    nans = np.empty(data.shape, dtype=float).fill(np.nan)
...
def circ_kern(diameter):
    ...
    return np.less_equal(rads, r).astype(int)
...
def fast_hough(in_arr, xyt):  
    ...
    '''
    if hout == None:
        return np.einsum('ijk,ij', xyt, in_arr) #, dtype=int) 
    else:
        assert hout.ndim == 1
    ...
    # cube = np.multiply( in_arr.reshape((in_arr.shape[0],in_arr.shape[1],1)), xyt).astype(float, copy=False)
    ...
    # return np.sum(np.sum( cube , axis=0, dtype=int), axis=0, dtype=float) #WORKS FAST AND DIVIDES PROPERLY
    # return np.sum(cube, axis=(1,0), dtype=int)
...
def houghnew(image, cos_theta, sin_theta):
    ...
    out = np.zeros((int(nr_bins), len(cos_theta)), dtype=int)
...
def all_thetas(wlen, theta, original):
    ...
    out = np.zeros(window.shape+(ntheta,), int)
...
def interpret(filepath, force=False, wlen=WLEN, frac=FRAC, smr=SMR, original=ORIGINAL):
    ...
    data = getData(filepath)
    backproj = np.load(backproj_filename).astype(float)
    spectrum = np.load(spectrum_filename).astype(float)
